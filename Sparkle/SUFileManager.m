...
- (BOOL)_removeQuarantineWithAuthenticationAtRootURL:(NSURL *)rootURL error:(NSError *__autoreleasing *)error
{
    ...
    if (![_fileManager fileExistsAtPath:(NSString * _Nonnull)@(XATTR_UTILITY_PATH)]) {
        if (error != NULL) {
...
- (BOOL)moveItemAtURLToTrash:(NSURL *)url error:(NSError *__autoreleasing *)error
{
    ...
    if (!canUseNewTrashAPI) {
        ...
        NSString *name = tempItemURL.lastPathComponent;
        if (name) {
            success = [[NSWorkspace sharedWorkspace] performFileOperation:NSWorkspaceRecycleOperation source:tempParentPath destination:@"" files:@[name] tag:NULL];
        }
        if (!success && error != NULL) {
